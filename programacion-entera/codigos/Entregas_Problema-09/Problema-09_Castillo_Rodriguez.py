# -*- coding: utf-8 -*-
"""
Created on Sat Aug  4 22:39:41 2018
...
for c in prob.variables():
    if c.name=="x_barra":
        xvalor_CM=c.name,"=", c.varValue
    if c.name=="y_barra":
        yvalor_CM=c.name,"=", c.varValue
print("Centro de masa x", xvalor_CM)
print("Centro de masa y", yvalor_CM)
xerror=(xvalor_CM[2]-x_tilde)
yerror=(yvalor_CM[2]-y_tilde)
costo_t=(xerror+yerror)*(-1)

print("En x la diferencia que hay entre la locacion centro de masa de contenedores y locacion centro de masa deseada es:", xerror)
print("En y la diferencia que hay entre la locacion centro de masa de contenedores y locacion centro de masa deseada es:", yerror)
print("El costo total es:", costo_t)
