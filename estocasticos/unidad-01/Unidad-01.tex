...
\begin{itemize}
\item Tenemos dos tipos, denotadas $\Geo(p)_0$ y $\Geo(p)_1$. 
\item La variable aleatoria $\Geo(p)_0$ representa el n\'umero de ensayos que tenemos que realizar hasta obtener el primer experimento exitoso. 
\begin{itemize}
...
\item La variable aleatoria $\Geo(p)_1$ representa el \'indice del ensayo que result\'o \linebreak en el primer experimento exitoso. 
\begin{itemize}
...
\]
\item \textbf{Teorema:} Para cada \'indice $i$ es el caso que $T_i \sim \Geo(p)_0$. 
\item \textbf{Corolario:} En un proceso Bernoulli con par\'ametro $p$ los tiempos entre arribos constituyen una secuencia de variables aleatorias independientes e identicamente distribuidas; en particular, con distribuci\'on $\sim \Geo(p)_0$. 
\end{itemize}
...
N_k \, = \, \sum_{t=1}^k X_t
\]
\item \Ie la variable aleatoria $N_k$ es la suma de $k$ variables aleatorias Bernoulli independientes e identicamente distribuidas (IID). 
\item \textbf{Teorema:} Para cada \'indice $k$ es el caso que $N_k \sim \Binomial(k,p)$. 
\item \textbf{Corolario:} En un proceso Bernoulli con par\'ametro $p$ el n\'umero de arribos \linebreak a lo largo de un intervalo de $k$ per\'iodos es una variable aleatoria con distribuci\'on binomial con par\'ametros $k$ y $p$. 
\end{itemize}
...
Propiedades estad\'isticas: 
\begin{itemize}
\item \textbf{Falta de memoria:}
\[
\forall \, k \geq 0, \, 
\forall \, \tau_1, \tau_2 > 0 \; \colon \;
\Pr( \, X_k > \tau_1 + \tau_2 \mid X_k > \tau_1 \, ) \; = \; 
\Pr( \, X_k > \tau_2 \, )
\]
\item \textbf{N\'umero de arribos en un intervalo:} \\ En un proceso Poisson con par\'ametro $\lambda$ el n\'umero de arribos en un intervalo de $\tau$ unidades es una variable aleatoria Poisson con par\'ametro $\lambda \, \tau$. 
\item \textbf{Arribos en intervalos disjuntos:} \\ Si dos intervalos de tiempo son disjuntos entonces el n\'umero de arribos durante esos intervalos son estad\'isticamente independientes. 
\end{itemize}
